...
черги. **RB-40…43 landed** this `agi` drain. Next = RB-44.
...
1. **RB-44** — Studio page-view: наскрізна нумерація книги (зараз per-chapter);
   звірити ±1 з `interior_pdf`.
2. **RB-45** — hyphenation в interior (rustybuzz уже транзитна залежність).
3. **RB-46** — FOGRA39 ICC drop-in (`REBOOK_ICC_CMYK` уже читається, файл не
   в репо) + CMYK JPEG passthrough на X-1a wrap.
4. **RB-47** — eBook cover DPI ≥300 для print-art; EN OPF ISBN на живих книгах
   власника (відомі дірки з NEXT).
5. **RB-48** — віртуальний стенд: ebook «листання» (CSS pages), не лише front;
   paperback spread з recto/verso як у Studio V.
6. **RB-49** — AI-переклад чернетки (env-gated `/api/ai`, не хмара за замовчуванням).
7. **RB-50** — EPUBCheck-еквівалент v2 (зовнішні URL, script, ISBN per format).
