...
`rebook` — один портативний Rust-бінарник (`rust_book` / `rust_book.exe`).
Копіюєш у будь-яку теку, `init` + свій `book.json` + `chapters/`, без git і
без Cargo. Сервіс слухає лише `127.0.0.1:8090`: пишеш книгу, бачиш її як
читач і як друкарня, вантажиш на **Amazon KDP** без локального AZW3/KFX.

Дані живуть у теці бінарника (або `REBOOK_HOME` / `--dir`). Системні
каталоги Windows як home відхиляються. Шляхи розділів не можуть містити `..`.
