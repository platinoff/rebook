...
|-----|----------------|----------------------|-----------|
| **eBook** | EPUB 3.2 (`mimetype` STORED first, OPF + `nav.xhtml`, cover JPEG/PNG ≥1000 px, ISBN як `urn:isbn:` якщо є) | Полиця `/` + глава + `/view3d` mode `ebook` (лише лицьова) | front only, ratio ≥ 1.6:1, ideal 1600×2560 |
| **Paperback** | zip: `interior.pdf` + `cover-wrap.pdf` + barcode + `manifest.json` | `/view3d` mode `pb` + Studio page-view (V) | bleed 0.125″, spine `pages×paper`, gutter table |
| **Hardcover** | zip: interior + case-laminate wrap (KDP 2026: **без** cloth/dust на KDP) | `/view3d` mode `hc`; Ingram jacket — mode `dj` (експорт, не KDP) | wrap 0.51″, hinge 0.4″, pages 75–550 even |
...
hinge, flaps) з EPUB-обкладинки полиці — ebook front, paperback wrap,
hardcover case, Ingram jacket.
