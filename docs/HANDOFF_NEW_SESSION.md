...
- **Latest work**: **RB-40…43 landed** (this `agi` drain). Cover magic-byte sniff;
  OPF `urn:isbn:` + kdp.rs no AZW3; GSV-style nav `data-tip` + uk/en chrome;
  write-then-translate fork (`POST /api/drafts/{id}/translate/{lang}`). Prior:
  bands 233–263, RB-1…RB-38. Three KDP types = ebook / paperback / hardcover;
  virtual stand = `/view3d` (CSS 3D — Vulkan researched and rejected).
  Gate: fmt · clippy 0 · **106** tests.
...
- **Dev plan / bands**: [`REBOOK_DEV_PLAN.md`](./REBOOK_DEV_PLAN.md);
  tickets on GSV board product `rebook` (hub `:9999` was down this drain —
  fail-open; next `agi` re-hooks the board).
