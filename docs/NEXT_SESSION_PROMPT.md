...
1. **Owner product pick: rebook.** Window `S:/rust/rebook`. `agi` = keep-live +
   tickets then RB-51 (SVG plates for Classic American Iron).
2. **State: RB-40…50 landed.** RB-50 = coloring paperback canon
   ([`COLORING_CARS.md`](./COLORING_CARS.md): 8.5×11, 24 cars, 46 plates, 102 p).
...
4. **Next band = RB-51:** SVG plate kit (stroke ≥0.75 pt, custom mark, captions).
   Then RB-52 Studio draft → RB-53 wrap → RB-54 3D → RB-55 preflight before publish.
