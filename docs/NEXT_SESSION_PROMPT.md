...
1. **Owner product pick: rebook.** Window `S:/rust/rebook`. `agi` = keep-live +
   tickets then RB-52 (Studio draft for Classic American Iron).
2. **State: RB-40…51 landed.** SVG plates via `cargo run -- coloring-plates`.
...
4. **Next band = RB-52:** Studio draft `uk` + SVG includes; ⇄ `en`.
   Then RB-53 wrap → RB-54 3D → RB-55 preflight before publish.
