...
1. **Owner product pick: rebook.** Window `S:/rust/rebook`. `agi` = keep-live +
   tickets then RB-50.
2. **State: RB-40…49 landed** (cover sniff, OPF ISBN, GSV nav chrome,
   ...
   verso/recto spread, AI `translate` + GSV box fullscreen). Prior RB-1…RB-38. App = portable web on
...
4. **Next band = RB-50:** EPUBCheck-еквівалент v2 (зовнішні URL, script, ISBN per format).
